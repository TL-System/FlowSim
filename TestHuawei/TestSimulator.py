...
opts, args = getopt.getopt(sys.argv[1:], "S:L:a:f:routing:i:server:tor:core:nn:expl:eps:reward:features:setting:max", ["S=", "L=", "a=", "f=", "routing=", "input=", "server=", "tor=", "core=", "nn=", "expl=", "eps=", "reward=", "features=", "setting=", "max="])
for o, a in opts:
    if o in ("-S", "--S"):
        flowSize = float(a)
    elif o in ("-L", "--L"):
...
def main():
    ...
    sim.AssignRoutingEngine(max_episodes, reward_type, features, number_hidden_nodes_per_layer, exploration_type, epsilon, setting, Routing=routing_scheme)
